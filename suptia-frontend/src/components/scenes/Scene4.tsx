...
export default function Scene4() {
  ...
  useLayoutEffect(() => {
    console.log("Scene4 useLayoutEffect triggered.");
    let ctx = gsap.context(() => {
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          scrub: true,
          onUpdate: (self) => console.log("Scene4 ScrollTrigger progress:", self.progress),
        },
      });
  ...
  return (
    <div ref={component} className="h-screen w-full relative bg-white text-black">
      <div ref={headlineRef} className="absolute inset-0 z-10 flex items-center justify-center text-center pointer-events-none opacity-0">
        <h2 className="text-5xl font-bold">すべての提案に、科学という裏付けを。</h2>
      </div>
    </div>
  )
}